...
static unsigned char const utfmask[UTF_SIZ + 1] = { 0xC0, 0x80, 0xE0, 0xF0,
						    0xF8 };
static long const utfmin[UTF_SIZ + 1] = { 0, 0, 0x80, 0x800, 0x10000 };
static long const utfmax[UTF_SIZ + 1] = { 0x10FFFF, 0x7F, 0x7FF, 0xFFFF,
					  0x10FFFF };
			...
			for (curfont = drw->fonts; curfont;
				...
				if (charexists) {
					...
					if (ew + tmpw > w) {
						overflow = 1;
						/* called from
                                                 * drw_fontset_getwidth)clamp(): it
                                                 * wants the width AFTER the
                                                 * overflow
                                                 */
						if (!render)
							x += tmpw;
		...
		if (!*text || overflow) {
			...
			if (!drw->fonts->pattern) {
				/* Refer to the comment in xfont_create for more
                                 * information. */
				die("the first font in the cache must be "
				    "loaded "
				    "from a font "
				    "string.");
			}

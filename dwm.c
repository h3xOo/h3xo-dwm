...
struct Client {
        ...
        int bw, oldbw;
        unsigned int tags;
        int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal,
            noswallow, issticky, ignoretransient;
        pid_t pid;
        Client *next;
        Client *snext;
        Client *swallowing;
...
static void pushstack(Arg const *arg);
...
static void sigdwmblocks(Arg const *arg);
#endif
static void sighup(int unused);
static void sigterm(int unused);
static void spawn(Arg const *arg);
static int stackpos(Arg const *arg);
...
static void togglescratch(Arg const *arg);
static void togglesticky(Arg const *arg);
static void togglefullscr(Arg const *arg);
...
void focus(Client *c)
{
        if (!c || !ISVISIBLE(c)) {
                for (c = selmon->stack;
                     c && (!ISVISIBLE(c) || (c->issticky && !selmon->sel->issticky)); c = c->snext)
                        ;
...
Atom getatomprop(Client *c, Atom prop)
{
        ...
        if (XGetWindowProperty(dpy, c->win, prop, 0L, sizeof atom, False, XA_ATOM, &da, &di, &dl,
                               &dl, &p)
                == Success
            && p) {
                atom = *(Atom *)p;
...
long getstate(Window w)
{
        ...
        if (XGetWindowProperty(dpy, w, wmatom[WMState], 0L, 2L, False, wmatom[WMState], &real,
...
#ifndef __OpenBSD__
void sigdwmblocks(Arg const *arg)
{
        union sigval sv;
...
void setclienttagprop(Client *c)
{
        ...
        XChangeProperty(dpy, c->win, netatom[NetClientInfo], XA_CARDINAL, 32, PropModeReplace,
                        (unsigned char *)data, 2);
}
...
void togglefullscr(Arg const *arg)
{
        (void)arg;
        if (selmon->sel)
...
void togglesticky(Arg const *arg)
{
        (void)arg;
        if (!selmon->sel)
                return;
...
void togglescratch(Arg const *arg)
{
        Client *c;
...
int updategeom(void)
{
        ...
        if (XineramaIsActive(dpy)) {
                ...
                for (i = 0, m = mons; i < nn && m; m = m->next, i++)
                        if (i >= n || unique[i].x_org != m->mw || unique[i].y_org != m->my
...
pid_t winpid(Window w)
{
        ...
        if (XGetWindowProperty(dpy, w, XInternAtom(dpy, "_NET_WM_PID", 0), 0, 1, False,
                               AnyPropertyType, &type, &format, &len, &bytes, &prop)
                != Success
            || !prop)
                return 0;

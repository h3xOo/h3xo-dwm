...
struct Client {
        ...
        int bw, oldbw;
        unsigned int tags;
        int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, issticky, ignoretransient;
        pid_t pid;
        Client *next;
        Client *snext;
        Client *swallowing;
...
void focusstack(Arg const *arg)
{
        ...
        for (p = NULL, c = selmon->clients; c && (i || !ISVISIBLE(c)); i -= ISVISIBLE(c) ? 1 : 0, p = c, c = c->next)
                ;
        focus(c ? c : p);
        restack(selmon);
}
...
void manage(Window w, XWindowAttributes *wa)
{
        ...
        updatewmhints(c);
        {
                ...
                Monitor *m;
                Atom atom;
                if (XGetWindowProperty(dpy, c->win, netatom[NetClientInfo], 0L, 2L, False, XA_CARDINAL, &atom, &format, &n, &extra,
...
int updategeom(void)
{
        ...
        if (XineramaIsActive(dpy)) {
                ...
                for (i = 0, m = mons; i < nn && m; m = m->next, i++)
                        if (i >= n || unique[i].x_org != m->mw || unique[i].y_org != m->my || unique[i].width != m->mw ||
...
pid_t winpid(Window w)
{
        ...
        if (XGetWindowProperty(dpy, w, XInternAtom(dpy, "_NET_WM_PID", 0), 0, 1, False, AnyPropertyType, &type, &format, &len, &bytes,
                               &prop) != Success ||
            !prop)
                return 0;
